...
const (
	azureWssUrl    = `wss://eastus.api.speech.microsoft.com/cognitiveservices/websocket/v1?TricType=AzureDemo&Authorization=bearer%20undefined&X-ConnectionId=`
	azureVoicesUrl = `https://eastus.api.speech.microsoft.com/cognitiveservices/voices/list`
)
...
func (t *AzureTTS) NewConn() error {
	log.Println("创建WebSocket连接(Azure)...")
	if t.WriteTimeout == 0 {
	...
	t.conn, resp, err = dl.DialContext(ctx, azureWssUrl+t.uuid, header)
	if err != nil {
		if resp == nil {
			return err
		}
...
func (t *AzureTTS) sendConfigMessage(format string) error {
...
func (t *AzureTTS) sendSsmlMessage(ssml string) error {
...
func GetAzureVoices() ([]byte, error) {
	req, err := http.NewRequest(http.MethodGet, azureVoicesUrl, nil)
	if err != nil {
		return nil, err
	}
